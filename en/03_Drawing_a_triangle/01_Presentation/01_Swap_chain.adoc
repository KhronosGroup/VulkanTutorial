...
This is a complex topic that we'll learn more about in link:!en/Drawing_a_triangle/Swap_chain_recreation[a future chapter].
...
If at this point you get an access violation error in `vkCreateSwapchainKHR` or see a message like `Failed to find 'vkGetInstanceProcAddress' in layer SteamOverlayVulkanLayer.dll`, then see the link:!en/FAQ[FAQ entry] about the Steam overlay layer.
...
link:/code/06_swap_chain_creation.cpp[C{pp} code]
