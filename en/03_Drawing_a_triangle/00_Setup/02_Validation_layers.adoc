...
We'll simply specify the same layers as the instance at logical device level, which we'll see link:!en/Drawing_a_triangle/Setup/Logical_device_and_queues[later on].
...
Now it's time to look at link:!en/Drawing_a_triangle/Setup/Physical_devices_and_queue_families[Vulkan devices in the system].

link:/code/02_validation_layers.cpp[C{pp} code]
